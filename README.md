...
- **今後に向けた改善**
## React + DRFへの移行を進める
開発時、Reactについて知識不足だったことと、勉強のためまずフレームワークを使わずに書きたかったという理由から
Reactを選定しなかった。
だが現状、フロントエンドがReactを用いず書かれているため、コードの可読性が低く、改善しずらいという課題がある。
そのため、Reactの導入によるフロントのコンポーネント化と、それに伴いバックエンドのDRFを使ったAPIサーバー化に着手していく。
